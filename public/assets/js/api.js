...
function searchCharacterById() {
  ...
  api
    ...
    .then((res) => {
      ...
      document.getElementById("replaceThumbnailResults").src =
      ...
      document.getElementById(
        "replaceDescriptionResults"
      ).innerHTML = `Description: ${desc} <a href="${url1}" target="blank" class="text-center">More info <u>here</u></a>.`;
      ...
      for (const comic of comicsList) {
        ...
        document.getElementById("comicsList").innerHTML += `
                ...
                <a href="${comic.urls[0].url}" target="blank">
                    <img id="comicsResultImage${x}"  src="${thumb}" class="card-img-top" alt="${
          comic.title
        }"></a>
